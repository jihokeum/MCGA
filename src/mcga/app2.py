...
#---------------------------------------------
    ...
    # ── 8) TOXICITY & BY-PRODUCTS SUMMARY ─────────────────────────
    st.subheader("Toxicity Summary")

    # 1) Gather by-product SMILES (everything except the target)
    byproduct_smiles = [
    ...
    # 2) Flatten all hazard codes from by-products
    all_codes = []
    byproduct_pics = []
    for smi in byproduct_smiles:
        codes = get_ghs_data(smi) or []
        pics  = hazard_statements(smi) or []
        all_codes.extend(codes)
        byproduct_pics.extend(pics)

    # 3) Dedupe pictograms
    byproduct_pics = list(dict.fromkeys(byproduct_pics))

    # 4) Compute acute/CMR code counts just like before
    ...
    severity_value = len(acute_codes) + (4 if carcino_codes else 0)
    if carcino_codes:
        detail_comment = (
            f"Acute GHS codes: {len(acute_codes)}\n"
            f"CMR GHS codes: {len(carcino_codes)}\n"
            "4+ acute codes or any CMR → serious hazard!"
        )
    elif not acute_codes:
        detail_comment = "No acute or CMR codes found – excellent safety profile."
    elif len(acute_codes) <= 3:
        detail_comment = (
            f"Acute GHS codes: {len(acute_codes)}\n"
            "1–3 acute codes – moderate risk."
        )
    else:
        detail_comment = (
            f"Acute GHS codes: {len(acute_codes)}\n"
            "4+ acute codes – serious hazard!"
        )
    ...
    with col1:
        st.subheader("By-Product Pictograms")
        if byproduct_pics:
            icons = [str(ASSETS_DIR / PICTO_MAP[p]) for p in byproduct_pics if p in PICTO_MAP]
            st.image(icons, width=64, caption=byproduct_pics)
        else:
            st.write("_No hazard pictograms for by-products_")
    ...
    with col2:
        display_metric_feedback(
            ...
            display_value=f"{len(acute_codes)} acute, {len(carcino_codes)} CMR codes"
        )
